...
def save_spatial_grid_to_geojson(estimation_array_file, ground_truth_array_file, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, outfile):
        ...
        for i in range(array_shape[0]):
            for j in range(array_shape[1]):
                for k in range(array_shape[2]):
                    ...
                    error = ground_truth_array[i, j, k] - estimation_array[i, j, k]
                    my_polygon = Polygon([[(x, y), (x+x_unit_degree, y), (x+x_unit_degree, y+y_unit_degree), (x, y+y_unit_degree), (x, y)]])
                    my_feature = Feature(geometry=my_polygon, properties={"timestamp": t, "estimation": estimation, "ground_truth": ground_truth, "error": error})
...
def save_spatial_grid_to_geojson_timeslice(estimation_array_file, ground_truth_array_file, tweet_array_file, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, outfile, slice_timestart):
    estimation_array = numpy.load(estimation_array_file, allow_pickle=True)
    ground_truth_array = numpy.load(ground_truth_array_file, allow_pickle=True)
                ...
                for k in range(array_shape[2]):
                    ...
                    if t == slice_timestart:
                        ...
                        if num_tweet > 0:
                            ...
                            y = aoi[1] + (y_unit_degree * k)
                            estimation = estimation_array[i, j, k]
                            ground_truth = int(ground_truth_array[i, j, k])
                            error = ground_truth_array[i, j, k] - estimation_array[i, j, k]
                            my_polygon = Polygon([[(x, y), (x+x_unit_degree, y), (x+x_unit_degree, y+y_unit_degree), (x, y+y_unit_degree), (x, y)]])
                            my_feature = Feature(geometry=my_polygon, properties={"timestamp": t, "estimation": estimation, "ground_truth": ground_truth, "error": error, "num_tweet": num_tweet})
                            my_feature_list.append(my_feature)
        my_feature_collection = FeatureCollection(my_feature_list)
        dump = geojson.dumps(my_feature_collection, sort_keys=True)
        geojson_file.write(dump)
...
def save_spatial_grid_to_geojson_global_mean(estimation_array_file, ground_truth_array_file, tweet_array_file, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, outfile):
    ...
    error = numpy.subtract(ground_truth_array, estimation_array)
...
if __name__ == '__main__':
    ...
    # estimation = numpy.load(ESTIMATION_LSI_GB_FILE, allow_pickle=True)
    # zero_index = numpy.where(tweet_counter == 0)
    # estimation[zero_index] = 'nan'
    # error = numpy.subtract(gps_counter, estimation)
    # error = numpy.square(error)
    # error = numpy.sqrt(error)
    ...
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130725_1900, ESTIMATION_HANABI_20130725_1900_TIME)
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130726_1900, ESTIMATION_HANABI_20130726_1900_TIME)
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130727_1900, ESTIMATION_HANABI_20130727_1900_TIME)
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130728_1900, ESTIMATION_HANABI_20130728_1900_TIME)
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130729_1900, ESTIMATION_HANABI_20130729_1900_TIME)
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130730_1900, ESTIMATION_HANABI_20130730_1900_TIME)
    # save_spatial_grid_to_geojson_timeslice(ESTIMATION_LSI_GB_FILE, GPS_COUNTER_FILE, TWEET_COUNTER_FILE, aoi, x_unit_degree, y_unit_degree, timestart, unit_temporal, ESTIMATION_HANABI_20130731_1900, ESTIMATION_HANABI_20130731_1900_TIME)
